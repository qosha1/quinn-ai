"""
GeminiSession - SessionInterface implementation for Google Gemini CLI.

Wraps shared/pyterm's AgentSession to provide the SessionInterface contract
expected by the Worker class. Gemini is Google's AI assistant with support
for large context windows (up to 1M tokens).
"""
...
class GeminiSession(SessionInterface):
    """
    SessionInterface implementation for Google Gemini CLI.
    ...
    Gemini supports large context windows (up to 1M tokens), making it
    ...
    Example:
        ...
        session = GeminiSession(config, pyterm_config)
        session.start()
    ...
    def _spawn_process(self) -> None:
        """Spawn the Gemini CLI process via pyterm AgentSession."""
        try:
            # Create AgentSession config
            # Note: Gemini uses a "generic" parser since we don't have a gemini-specific parser yet
            agent_config = AgentSessionConfig.create(
                worker_id=self._config.worker_id,
                provider="generic",  # Use generic parser for Gemini
    ...
    def _detect_ready(self, output: str) -> bool:
        """Detect if Gemini is ready for input."""
        if not self._agent_session:
            return False
    ...
    def _get_context_usage(self) -> int:
        ...
        Note: Gemini supports up to 1M tokens, so this is particularly
        important for context management.
        """
    ...
    def max_context_tokens(self) -> int:
        """Maximum context tokens supported by Gemini.

        Gemini Pro supports up to 1M tokens.
        """
        return 1_000_000
